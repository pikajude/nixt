use super::{Inode, LocalStore, OptimiseStats};
use crate::{archive, prelude::*};
...
impl LocalStore {
  pub(super) fn canonicalise_path_metadata<P: AsRef<Path>>(
    &self,
    path: P,
    uid: Option<u32>,
  ) -> Result<()> {
  ...
  pub(super) fn canonicalize_timestamp_and_permissions<P: AsRef<Path>>(
    &self,
    path: P,
    info: &fs::Metadata,
  ) -> Result<()> {
  ...
  pub(super) fn optimise_path(&self, path: &Path) -> Result<()> {
